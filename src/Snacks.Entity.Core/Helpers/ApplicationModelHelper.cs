// Helpers created from aspnetcore source
// https://github.com/dotnet/aspnetcore/blob/main/src/Mvc/Mvc.Core/src/ApplicationModels/DefaultApplicationModelProvider.cs
...
namespace Snacks.Entity.Core.Helpers
{
    internal static class ApplicationModelHelper
    {
        ...
        public static ControllerModel CreateControllerModel(
            ...
            IModelMetadataProvider modelMetadataProvider)
        {
            ...
            foreach (var propertyInfo in controllerModel.ControllerType.AsType().GetProperties())
            {
                var propertyModel = CreatePropertyModel(propertyInfo, modelMetadataProvider);
            ...
            foreach (var methodInfo in controllerModel.ControllerType.AsType().GetMethods())
            {
                var actionModel = CreateActionModel(controllerModel.ControllerType, methodInfo);
                if (actionModel == null)
                {
                    continue;
                }
                ...
                {
                    var parameterModel = CreateParameterModel(parameterInfo, modelMetadataProvider);
        ...
        private static PropertyModel CreatePropertyModel(PropertyInfo propertyInfo, IModelMetadataProvider modelMetadataProvider)
        {
            ...
            var modelMetadata = modelMetadataProvider.GetMetadataForProperty(declaringType, propertyInfo.Name);
        ...
        /// </remarks>
        private static bool IsAction(TypeInfo typeInfo, MethodInfo methodInfo)
        {
            if (typeInfo == null)
            {
                throw new ArgumentNullException(nameof(typeInfo));
        ...
        private static ParameterModel CreateParameterModel(ParameterInfo parameterInfo, IModelMetadataProvider modelMetadataProvider)
        {
